...
const photosGallery = (title, dimensions = "1920x1080", date) => {
  return {
    title,
    date,
    [dimensions]: true,
    ...
    dimensions,
    publishInfo() {
      console.log(`Фото "${title}" было опубликовано ${Math.floor((new Date().getTime() - date.getTime()) / 1000)
    } секунды назад`)
    },
  }
}
...
Потому что там в функции photosGallery создан объект, ссылку на который присвоили к переменной
myDogPhoto. Соответсвенно все значения параметров храняться в этом объекте  */
