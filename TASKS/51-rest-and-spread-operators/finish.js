 ...
 */
const meanScore = (...meanArray) => {
  if (meanArray.some((num) => typeof num !== 'number')) {
    console.error('Все аргументы в вызове функции должны быть числами!')
  }
  return (meanArray.reduce((sumNum, num) => sumNum + num / meanArray.length, 0)).toFixed(2)
  // return numbers
  //   .reduce((mean, num) => mean + num / numbers.length, 0)
  //   .toFixed(2)
  // начиная с точки можно можно опустить на строку ниже, как тут
}
